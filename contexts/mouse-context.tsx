...
export function MouseProvider(props: { children: React.ReactNode }) {
  ...
  function hideCustomCusor(cursor: React.MutableRefObject<HTMLDivElement | null>) {
    if (cursor && cursor.current) {
      cursor.current.style.opacity = '0';
    }
  }
  function showCustomCusor(cursor: React.MutableRefObject<HTMLDivElement | null>) {
    if (cursor && cursor.current) {
  ...
  return <MouseContext.Provider value={mouseProvider}>{props.children}</MouseContext.Provider>;
}
